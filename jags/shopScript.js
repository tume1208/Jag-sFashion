document.addEventListener("DOMContentLoaded", function() {
    ...
    if (searchInput) {
       ...
       function displayAllProducts(category) {
           ...
           // Show filters for the selected category
           document.querySelectorAll(".filters").forEach(filter => {
               filter.style.display = "none";
           });
           const filters = document.querySelector(`#${category}-products .filters`);
           if (filters) {
               filters.style.display = "block";
           } else {
               console.error(`Filters not found for category: ${category}`);
           }
   
           // Add event listeners for filters
           const priceFilter = document.querySelector(`#${category}-products #${category}-price-filter`);
           const materialFilter = document.querySelector(`#${category}-products #${category}-material-filter`);
           const styleFilter = document.querySelector(`#${category}-products #${category}-style-filter`);
   
           if (priceFilter && materialFilter && styleFilter) {
               ...
               console.error(`One or more filters not found for category: ${category}`);
           }
       }
       ...
       function applyFilters(category) {
           ...
           const priceFilter = document.querySelector(`#${category}-products #${category}-price-filter`).value;
           const materialFilter = document.querySelector(`#${category}-products #${category}-material-filter`).value;
           const styleFilter = document.querySelector(`#${category}-products #${category}-style-filter`).value;
   
           console.log("Applying filters:", { priceFilter, materialFilter, styleFilter });
           ...
           if (styleFilter !== "all") {
               filteredProducts = filteredProducts.filter(product => product.style === styleFilter);
           }
